...
//get video from YT Channel
$(document).ready(function(){function d(){$.ajax({url:"https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=PL2ZbFswbAD-_nfD_3Qy99zw7c1lOBacQE&key=AIzaSyALiTeDemN-zjrulrD723vdt6MIjRfIMuY",type:"get",dataType:"jsonp",success:function(a){var b=[];a=a.items;if(void 0!==a){for(var c=0;c<a.length;c++)b.push(a[c].snippet.resourceId.videoId);0<b.length&&(a=Math.floor(Math.random()*b.length),b="http://www.youtube.com/embed/"+b[a]+"?rel=0&amp;autoplay=1&amp;wmode=transparent",$(".yt-overlay-play-button a").attr("href",
b),$(".yt-overlay-play-button a").addClass("has-yt"));$(".yt-overlay-play-button a[data-lightbox-gallery]").iLightbox()}},error:function(a){console.log(a)}})}d();$(".yt-overlay-play-button a").on("click",function(){d()});null!=Cookies.get("confirmage")&&$(".switch-input").prop("checked",!0);$(".switch").on("click",function(){$(".switch-input").is(":checked")?(null==Cookies.get("confirmage")&&Cookies.set("confirmage",1,{path:"/",domain:"sexygirlmedia.blogspot.com",secure:!1}),0<$(".main-post-content .adult-content").length&&
...
$.removeCookie("confirmage",{expires:null,path:"/",domain:"sexygirlmedia.blogspot.com",secure:!1})})});
