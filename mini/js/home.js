...
function getNewVideo(callback){
	$.ajax({
		url: url_blog + "/feeds/posts/summary/-/Video?alt=json-in-script&orderby=published&max-results=40",
		type: "GET",
		dataType: "jsonp",
			...
			if(entry !== undefined){
				...
				for(var i=0; i<entry.length; i++){
					...
					if("media$thumbnail" in entry[i]){
						...
						thumbPost = thumbPost.replace(/\/s[0-9]+(\-c)\/?/g, "/w300-h225-c")
					}else{
		...
		error: function(){
			newestInsert = '<strong>Error Loading Feed!</strong>';
		}
	});
}
