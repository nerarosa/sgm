...
var urlDB = 'http://sgmgirldata.blogspot.com';
...
function getGirlList(){
	if(cat != ''){
		var url = urlDB + '/feeds/posts/default/-'+ cat +'?alt=json-in-script&orderby=published&max-results=' + postInPage + '&start-index=' + startIndex;
	}else
		var url = urlDB + '/feeds/posts/default?alt=json-in-script&orderby=published&max-results=' + postInPage + '&start-index=' + startIndex;
		...
		success: function(data){
			...
			if(entry !== undefined){
				for(var i = 0; i < entry.length; i++){
					...
					if(sAvt == '' || sAvt == null) sAvt = 'http://placehold.it/200x200?text=No+Image';
				...
				$('.mote-all ul').html('<li><div class="mote-list-rows-img-di"><a href="#"><img alt="No Image" src="http://placehold.it/200x200?text=No+Image"/></a></div><a href="#" class="mote-list-rows-title">No Girl</a></li>');
			}
		},
		error: function(){
			setTimeout(function(){
...
function paginationG(){
	if(cat != ''){
		var url = urlDB + '/feeds/posts/default/-'+ cat +'?alt=json-in-script&max-results=0';
	}else
		var url = urlDB + '/feeds/posts/default?alt=json-in-script&max-results=0';
...
function getGirlInfo(id, count){
	var url = urlDB + '/feeds/posts/default/'+ id +'?alt=json-in-script';
...
function getNewUpdateList(){
	var url = url_blog + 'feeds/posts/summary/-/sgmgirl?alt=json-in-script&orderby=published&max-results=200';
	$.ajax({
		type: 'get',
		url: url,
		dataType: 'jsonp',
		success: function(data){
			...
			if(entry !== undefined){
				...
				for(var i = 0, len = tempIdArr.length; i < len; i++){
					if(requestMobile == false)
...
function getGirlInfo(id){
	...
	$.ajax({
		...
		success: function(data){
			...
			if(avt == '' || avt == null) avt = 'http://placehold.it/200x273?text=No+Image';
			if(cover == '' || cover == null) cover = 'http://placehold.it/1054x241?text=No+Image';
...
function getAllPostByGirl(id){
	var url = url_blog + 'feeds/posts/summary?q=@'+ id +'&alt=json-in-script&orderby=published&max-results=20';
	$.ajax({
		url: url,
		type: "get",
		dataType: "jsonp",
		success: function(data){
			...
			if(typeof entry !== undefined){
				for(var i = 0, len = entry.length; i < len; i++){
					...
					thumb = "media$thumbnail" in entry[i] ? entry[i].media$thumbnail.url.replace('/s72-c/', '/w250-h360-c/') : "http://placehold.it/250x360?text=No+Image";
					
					var date = new Date(entry[i].published.$t);
						datePost = date.getDate() + '-' + (date.getMonth() + 1) + '-' + date.getFullYear();
					
					if(requestMobile == false)
						htmlEmbed += '<dl><dd class="mote-list-body-rows-img"><a target="_blank" href="' + urlPost + '"><img alt="'+ title +'" title="'+ title +'" data-original="'+ thumb +'" src="http://placehold.it/250x360?text=SGMedia"/></a></dd><dt><a target="_blank" href="'+ urlPost +'" title="'+ title +'" >' + title + '</a></dt><dd class="mote-list-body-rows-more clearfix"><span class="mote-icon-time">'+ datePost +'</span><span class="mote-icon-eye">0</span></dd></dl>';
					else
						htmlEmbed += '<li><a href="' + urlPost + '" title="'+ title +'"><img data-original="'+ thumb +'" src="http://placehold.it/250x360?text=SGMedia" alt="'+ title +'"/></a><a class="title" href="'+ urlPost +'">'+ title +'</a></li>';
				}
...
function paginationGInfo(id){
	var url = url_blog + 'feeds/posts/default?q=@'+ id +'&alt=json-in-script&max-results=0';
...
function getTopGirl(id){
	...
	$.ajax({
		...
		success: function(data){
			...
			if(avt == '' || avt == null) avt = 'http://placehold.it/190x190?text=No+Image';			
			if(requestMobile == false){
	...
	postsRefPhoto.once('value', function(snapshot) {
		...
		if(snapshot.exists()){
			...
			for(var i = 0; i < data.length; i++){
				htmlEmbed += '<li><div class="mote-hot-imgs-body-pic"><a target="_blank" href="'+ data[i].url +'"><img alt="'+ data[i].title +'" title="'+ data[i].title +'" src="'+ data[i].thumb.replace('/s72-c/', '/w250-h360-c/') +'"/></a><span>Xem:'+ data[i].viewCount +'</span></div><a target="_blank" class="mote-hot-imgs-body-t" href="'+ data[i].url +'" title="'+ data[i].title +'" >'+ data[i].title +'</a></li>';
			}
